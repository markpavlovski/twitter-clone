...
class FrontEndController {
  constructor(topNav, mainContent, mainPost) {
    ...
    const clickPostScreenEsc = {
      action: 'click',
      el: this.mainPostScreen.querySelector('.post-card-header'),
      cb: this.hidePostScreen.bind(this)
    }
  ...
  editPost(event) {
    ...
    if (card) {
      ...
      textarea.value = post.messageBody
      textarea.setAttribute('postId',id)
  ...
  editSubmitPost() {
    const textarea = this.mainPostScreen.querySelector('textarea')
    const id = textarea.getAttribute('postId')
    const postData = this.stagePostData()
    console.log(postData);
    axios.post(`http://localhost:3000/messages/${id}`, postData)
    .then(()=>{
      this.renderFeed().then(()=>this.hidePostScreen())
    })
  }
